                                          ...
                                          enc_cb* cb_input)
{
  ...
  const SplitType split_type = get_split_type(ectx->get_sps().get(),
  ...
  */
  ...
  if (option_no_split) {
    ...
    *cb_input->downPtr = cb;
    ...
    if (split_type == OptionalSplit) {
      encode_split_cu_flag(&ectx->imgdata->ctbs,opt.get_cabac(), cb->x,cb->y, cb->ctDepth, 0);
  ...
  if (option_split) {
    ...
    *cb_input->downPtr = cb;
    ...
    // add rate for split flag
    if (split_type == OptionalSplit) {
      encode_split_cu_flag(&ectx->imgdata->ctbs,option_split.get_cabac(), cb->x,cb->y, cb->ctDepth, 1);
