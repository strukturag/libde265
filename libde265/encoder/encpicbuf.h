...
struct image_data
{
  ...
  const image* input; // owner
  image* prediction;  // owner
  image* reconstruction; // owner
...
class encoder_picture_buffer
{
 public:
  ...
  image_data* insert_next_image_in_encoding_order(const image*, int frame_number);
  ...
  void set_prediction_image(int frame_number, image*); // store it just for debugging fun
  void set_reconstruction_image(int frame_number, image*);
