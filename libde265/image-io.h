...
class ImageSource
{
 public:
  ...
  virtual LIBDE265_API image* get_image(bool block=true) = 0;
...
class ImageSink
{
 public:
  ...
  virtual LIBDE265_API void send_image(const image* img) = 0;
};
...
class ImageSink_YUV : public ImageSink
{
 ...
 LIBDE265_API ImageSink_YUV() : mFH(NULL) { }
  ...
  virtual LIBDE265_API void send_image(const image* img);
