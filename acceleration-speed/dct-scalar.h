...
public:
  ...
  virtual void runOnBlock(int x,int y) {
    memset(out,0,4*4);
    transform_4x4_add_8_fallback(out, xy2coeff(x,y), 4, 3,3);
  }
};
...
public:
  ...
  virtual void runOnBlock(int x,int y) {
    memset(out,0,8*8);
    transform_8x8_add_8_fallback(out, xy2coeff(x,y), 8, 7,7);
  }
};
...
public:
  ...
  virtual void runOnBlock(int x,int y) {
    memset(out,0,16*16);
    transform_16x16_add_8_fallback(out, xy2coeff(x,y), 16, 15,15);
  }
};
...
public:
  ...
  virtual void runOnBlock(int x,int y) {
    memset(out,0,32*32);
    transform_32x32_add_8_fallback(out, xy2coeff(x,y), 32, 31,31);
  }
};
